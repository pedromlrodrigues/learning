...
// 1.
dogs.reduce((dogs, curDog) => {
  const recommendedFood = Math.trunc(curDog.weight ** 0.75 * 28);
  curDog.recFood = recommendedFood;
  return dogs;
}, dogs);
...
// 2.
const dogSarah = dogs.find(x => x.owners.includes('Sarah'));
console.log(
  `Your dog is eating too ${
...
// 4.
const ownersEatTooMuchFormatted = ownersEatTooMuch.join(' and ');
const ownersEatTooLittleFormatted = ownersEatTooLittle.join(' and ');
console.log(`${ownersEatTooMuchFormatted}'s dogs eat too much!`);
console.log(`${ownersEatTooLittleFormatted}'s dogs eat too little!`);

// 5.
console.log(dogs.some(dog => dog.curFood === dog.recFood));

// 6.
const eatingOkayFormula = dog =>
  dog.curFood > dog.recFood * 0.9 && dog.curFood < dog.recFood * 1.1;
console.log(dogs.some(eatingOkayFormula));

// 7.
console.log(dogs.filter(eatingOkayFormula));
